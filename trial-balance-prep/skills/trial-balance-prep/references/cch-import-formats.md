...
2. **Fund names on the TB.** Some client TBs label the fund in a column or in
   ...
   the user.
3. **Caseware folder structure.** A Caseware export groups by nested folders,
   not by number — the fund lives in the tree, not the account string. Ask the
   user for a Caseware extract or a screenshot of the folder tree; read the
   two layers as **type › fund** (outer = fund type, inner = fund).

Always ask whether the engagement also needs a **standalone fund import**
(fund index / name / type) — `tb_io.write_fund_list()` writes it. That info
also comes from the Caseware folder tree.
