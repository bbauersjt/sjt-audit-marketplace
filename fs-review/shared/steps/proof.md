...
**Deterministic first.** `scripts/proof_scan.py` mechanizes the text-layer checks in
this module — TOC tie-out, printed page-number sequence, note numbering/references/
...
high-confidence misspellings. Adjudicate its `proof_scan.json` candidates
against the printed document first (each carries page + excerpt; candidates have
legitimate explanations — e.g., "the Plan" and "the Company" coexisting in an EBP
package). Then read the document for what only reading can catch: grammar and wording,
spelling the scan can't judge (proper nouns, capitalized terms), page breaks,
widows/orphans, spacing/alignment, whether a note reference's TARGET
actually covers the topic referenced, and anything else on the list below. Do not
re-derive by reading what the scan already checked mechanically.
...
- Collective reference to the entity — confirm the entity is consistently referred to as "the Company," "the Organization," "the District," "the Plan," "the Partnership," "the LLC," or similar, as applicable, throughout. Flag any inconsistency (e.g., switching between "the Company" and "the LLC" for the same entity).
