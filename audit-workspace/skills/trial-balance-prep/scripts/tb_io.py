...
def funds_from_cw_extract(funds):
    """Convert the caseware-crosswalk `funds` list (from _tb_extract.json) into
    CCH fund-import rows — the two CaseWare layers map onto CCH's two layers:
    non-root MASTERS -> fund types, CHILDREN -> funds.

    Each child carries `fund_type` (the ABBR of its nearest master ancestor,
    recovered from the ENTITY-key prefix hierarchy). Fund types are numbered
    '01', '02', ... in tree order. A child with no master ("orphan" — it sat
    directly under the engagement root in CaseWare) becomes its own fund type
    and is listed in `warnings` for the user to re-home.

    Returns {'rows': [{type_index, type_name, fund_index, fund_name}, ...],
             'warnings': [...]}.
    """
    masters = [f for f in funds if f.get("type") == "master"]
    children = [f for f in funds if f.get("type") == "child"]
    type_idx, type_name, rows, warnings = {}, {}, [], []
    used_types = set()
    for m in masters:
        type_idx[m["fund"]] = "%02d" % (len(type_idx) + 1)
        type_name[m["fund"]] = m["name"]
    for c in children:
        ft = c.get("fund_type", "")
        if not ft:  # orphan — promote to its own fund type
            if c["fund"] not in type_idx:
                type_idx[c["fund"]] = "%02d" % (len(type_idx) + 1)
                type_name[c["fund"]] = c["name"]
            ft = c["fund"]
            warnings.append("fund %s (%s) has no master in CaseWare - made it "
                            "its own fund type; re-home it if it belongs under "
                            "one" % (c["fund"], c["name"]))
        used_types.add(ft)
        rows.append({"type_index": type_idx[ft], "type_name": type_name[ft],
                     "fund_index": c["fund"], "fund_name": c["name"]})
    for m in masters:
        if m["fund"] not in used_types:
            warnings.append("master %s (%s) has no child funds - fund type %s "
                            "emitted with no funds under it"
                            % (m["fund"], m["name"], type_idx[m["fund"]]))
    rows.sort(key=lambda r: (r["type_index"], r["fund_index"]))
    return {"rows": rows, "warnings": warnings}


def write_fund_list(out_path, rows, sheet_name="Funds"):
    """Write the standalone fund-definition import — 4 required columns
    (mirrors the firm's fund-import example workbook):

        Fund Type Index | Fund Type Name | Fund Index | Fund Name

    rows: list of dicts with keys type_index, type_name, fund_index, fund_name
    (what funds_from_cw_extract returns). Index columns are written as text so
    leading zeros survive. Sterile, no freeze panes — same constraints as the
    TB import.
    """
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = sheet_name
    headers = ["Fund Type Index", "Fund Type Name", "Fund Index", "Fund Name"]
    for c, h in enumerate(headers, 1):
        ws.cell(row=1, column=c, value=h).font = Font(bold=True)
    for ri, f in enumerate(rows, 2):
        ws.cell(row=ri, column=1, value=str(f.get("type_index", "") or "")).number_format = "@"
        ws.cell(row=ri, column=2, value=f.get("type_name", ""))
        ws.cell(row=ri, column=3, value=str(f.get("fund_index", "") or "")).number_format = "@"
        ws.cell(row=ri, column=4, value=f.get("fund_name", ""))
    ws.freeze_panes = None
    wb.save(out_path)
    return {"funds": len(rows),
            "fund_types": len({(f.get("type_index"), f.get("type_name")) for f in rows})}
