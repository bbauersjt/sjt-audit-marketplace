...
> **Fresh groups before bulk assign.** Group UUIDs and
...
4. **▶ MANDATORY — reorder ascending.** Creates land newest-on-top (LIFO); without this they
   display in reverse index order. Pass the `list_financial_groups` rows straight through:
...
5. **Assign accounts** — GET the grouped TB rows, swap each row's group, batch in ONE call
   (sequential PATCHes crash the KC tab):
   ```python
...
6. **Verify** — re-GET the grouped TB. Assignment reads back at
   **`row.account.group.financialGroupId`** (NOT top-level `row.group`). Confirm
...
| Assignment "didn't apply" | read back at `row.group` (always stale) | check `row.account.group.financialGroupId` |
...
| `400 EngagementId` on assign PATCH | body missing the engagementId=clientId mapping | use rows from `get_trialbalance_grouped`, or `build_patch_body()` |
