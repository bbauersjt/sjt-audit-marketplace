...
- A single "do it all" JS call trips the in-page eval timeout (45s on the linked-tab CDP transport; the bridge's `chrome_eval` defaults to 30s) on a real program (AR is 56 parents / 147 step rows ≈ 300 writes), throwing a red "Runtime.evaluate timed out" error mid-run.
- The procedure is therefore split into **bounded, self-verifying steps** that each finish well under the timeout, with drops self-healing on re-verify.
- Never wrap it in one mega call.

Two hard platform facts force the shape:
1. **Writes must be paced — and pacing alone is NOT sufficient.** Back-to-back `UpdateProperty` to one form silently drop (HTTP 200, no persist). The old ~300ms floor is WRONG: ~60% still dropped at ~350ms, and ~30–50% drop even with 1–2s gaps. Pace ~1.2s AND run the mandatory write → per-workpaper submit → verify-by-read → retry ≤3× loop (field-conventions.md §5 3a) — which this module's verify+repair pipeline already embodies. → fire ≤ ~35 writes per call.
...
2. **TQs + visibility:** `await kcPop.tqAndVisible({binder, wp, area, tqYesCount, initials, gibberish})`. Answers the first N visible tailoring questions = Yes (the TQ→`IsApplicable` cascade then marks unneeded steps N/A) and sets the visible step-set to every applicable parent + applicable child. **The visible-set push is mandatory even when the program has ZERO TQs** — it is a required state push in its own right, not just a TQ-cascade side effect. Even when `TailoringQuestions`/`FlowTailoringQuestions` are both empty, ProgramSteps rows can start `visible:false`; the TQ loop no-ops but the `UpdateProgramStep` visible-set POST (`{binderId, workpaperId, value:"<;-joined step objectKeys>"}`, full-state) is still required. A `Risks` write on a step OUTSIDE the visible set does not register as a risk→step link — the plain form GET may echo it as state 3 / `isValueDefault:false` (phantom: the SPA renders "No steps have been linked" and "Risk Unaddressed" keeps firing). Push visibility → reload → rewrite `Risks`; cleared durably.
...
- **Sign-off property NAME varies per collection** (`performedby` vs `SignOff`) — read the
  actual `propertyType:3` key off the step before signing; never assume one name across forms.
- **Assertion checkboxes are CLIENT-RENDER-ONLY in the GET** — the `/api/Workpaper` GET reads them
  empty (same mechanic as KBA-502's bulk GET, which doesn't embed the RelevantAssertion child rows —
  they're cross-form bindings the SPA renders; KBA-502 still OWNS the IR/CR/RMM grid as the write
  target). Read a parent step's checked assertions from the **DOM**
  (`input[name="ProgramSteps|{key}|Assertion||{code}"].checked`) and write `Risks = RMM-{ASSERTION}`
  codes. DOM-first is mandatory for assertion linkage.
- **Risks property** = full-state replacement, `value`/`valueKey` semicolon-joined WITH trailing semi; only DOM-present codes (else 500 "Index was outside the bounds") — and only on steps already in the visible step-set (see step 2; a Risks write on an invisible step is a phantom that the plain form GET still echoes).
...
- **The "stick" fix covers step-edit flakiness too.** The same intermittent
