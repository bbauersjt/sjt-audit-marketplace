---
summary: Remove a stale document-level (WPM) sign-off from a workpaper or KC form — the bot-clears-its-own-stale-sign-off op. Applying sign-offs stays human-only.
leg: wpm
triggers:
  - "remove the sign-off"
  - "un-sign [form]"
...
status: validated
validated_on:
  - "Coop Consulting 2025 (NPO) — 2026-07-09, scripted removeSignOff on a throwaway preparer sign-off (200, verified gone via document_get)"
---
...
`POST /v1/signoff/removeSignOff`. This is the operation behind BB's fleet rule: when a bot
modifies a form, the prior sign-off is stale and comes off (KC's modified-after-signoff
...
- **Never remove another user's sign-off (out of bounds)** — the body has NO userId; removal is
  ...
  including a colleague's. The binder UI only lets you remove your OWN, so a bot must too
  (api-ui-parity, BB 2026-07-09). Step 1 is MANDATORY: confirm the leg's `userId` is the CURRENT
